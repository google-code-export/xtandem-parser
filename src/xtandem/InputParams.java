...
public class InputParams implements Serializable{
	...
	private double iMaxValidExpectValue = 0;
	...
	private double iProteinC_termResModMass = 0;
	private double iProteinN_termResModMass = 0;
	...
	private double iRefineMaxValidExpectValue = 0;
	private double iRefineModMass = 0;
	...
	private double iRefineTicPercent = 0;
	...
	private int iScoringMissCleavageSites = 0;
	private int iScoringMinIonCount = 0;
	...
	private double iSpectrumDynRange = 0;
	private String iSpectrumFragMassType = "";
	private double iSpectrumMonoIsoMassError = 0;
	private String iSpectrumMonoIsoMassErrorUnits = "";
	private int iSpectrumMaxPrecursorCharge = 0;
	private double iSpectrumMinFragMz = 0;
	private double iSpectrumMinPrecursorMz = 0;
	private int iSpectrumMinPeaks = 0;
	private double iSpectrumParentMonoIsoMassErrorMinus = 0;
	private double iSpectrumParentMonoIsoMassErrorPlus = 0;
	...
	private int iSpectrumSeqBatchSize = 0;
	private int iSpectrumThreads = 0;
	...
	public InputParams(HashMap map){
		...
		if (map.get("HISTOEXIST")!= null) iHistogramExist = convertStringToBool(map.get("HISTOEXIST").toString());
		if (map.get("LOGPATH")!= null) iLogPath = map.get("LOGPATH").toString();
		if (map.get("MAXVALIDEXPECT")!= null) iMaxValidExpectValue = Double.parseDouble(map.get("MAXVALIDEXPECT").toString());
		if (map.get("OUTPUTMESSAGE")!= null) iOutputMessage = map.get("OUTPUTMESSAGE").toString();
		if (map.get("ONESEQCOPY")!= null) iOneSequenceCopy = convertStringToBool(map.get("ONESEQCOPY").toString());
		if (map.get("OUTPUTPARAMS")!= null) iOutputParams = convertStringToBool(map.get("OUTPUTPARAMS").toString());
		if (map.get("OUTPUTPATH")!= null) iOutputPath = map.get("OUTPUTPATH").toString();
		if (map.get("OUTPUTPATHHASH")!= null) iOutputPathHash = convertStringToBool(map.get("OUTPUTPATHHASH").toString());
		if (map.get("OUTPUTPERFORMANCE")!= null) iOutputPerformance = convertStringToBool(map.get("OUTPUTPERFORMANCE").toString());
		if (map.get("OUTPUTPROTEINS")!= null) iOutputProteins = convertStringToBool(map.get("OUTPUTPROTEINS").toString());
		if (map.get("OUTPUTRESULTS")!= null) iOutputResults = map.get("OUTPUTRESULTS").toString();
		if (map.get("OUTPUTSEQPATH") != null) iOutputSequencePath = map.get("OUTPUTSEQPATH").toString();
		if (map.get("OUTPUTSEQUENCES")!= null) iOutputSequences = convertStringToBool(map.get("OUTPUTSEQUENCES").toString());
		if (map.get("OUTPUTSORTRESULTS")!= null) iOutputSortResults = map.get("OUTPUTSORTRESULTS").toString();
		if (map.get("OUTPUTSPECTRA")!= null) iOutputSpectra = convertStringToBool(map.get("OUTPUTSPECTRA").toString());
		if (map.get("OUTPUTSXSLPATH")!= null) iOutputXslPath = map.get("OUTPUTSXSLPATH").toString();;
		if (map.get("OUTPUTSXSLPATH")!= null) iProteinC_termResModMass = Double.parseDouble(map.get("C_TERMRESMODMASS").toString());
		if (map.get("N_TERMRESMODMASS")!= null) iProteinN_termResModMass = Double.parseDouble(map.get("N_TERMRESMODMASS").toString());
		if (map.get("C_TERMCLEAVMASSCHANGE")!= null) iProteinC_termCleavMassChange = map.get("C_TERMCLEAVMASSCHANGE").toString();;
		if (map.get("N_TERMCLEAVMASSCHANGE")!= null) iProteinN_termCleavMassChange = map.get("N_TERMCLEAVMASSCHANGE").toString();;
		if (map.get("CLEAVAGESITE")!= null) iProteinCleavageSite = map.get("CLEAVAGESITE").toString();;
		if (map.get("HOMOLOGMANAGE")!= null) iProteinHomologManagement = convertStringToBool(map.get("HOMOLOGMANAGE").toString());
		if(map.get("MODRESMASSFILE") != null) iProteinModifiedResMassFile = map.get("MODRESMASSFILE").toString();;
		if(map.get("TAXON") != null) iProteinTaxon = map.get("TAXON").toString();;
		if(map.get("REFINE") != null) iRefine = convertStringToBool(map.get("REFINE").toString());
		if(map.get("REFINEMAXVALIDEXPECT") != null) iRefineMaxValidExpectValue = Double.parseDouble(map.get("REFINEMAXVALIDEXPECT").toString());
		if(map.get("REFINEMODMASS") != null) iRefineModMass = Double.parseDouble(map.get("REFINEMODMASS").toString());
		...
		if(map.get("REFINETIC") != null) iRefineTicPercent = Integer.parseInt(map.get("REFINETIC").toString());
		...
		if(map.get("SCORING_CIONS")!=null) iScoring_cIons = convertStringToBool(map.get("SCORING_CIONS").toString());
		if(map.get("SCORINGCYCLPERM")!=null) iScoringCyclicPerm = convertStringToBool(map.get("SCORINGCYCLPERM").toString());
		if(map.get("SCORINGINCREV")!=null) iScoringIncReverse = convertStringToBool(map.get("SCORINGINCREV").toString());
		if(map.get("SCORINGMISSCLEAV")!=null) iScoringMissCleavageSites = Integer.parseInt(map.get("SCORINGMISSCLEAV").toString());
		if(map.get("SCORINGMINIONCOUNT")!=null) iScoringMinIonCount = Integer.parseInt(map.get("SCORINGMINIONCOUNT").toString());
		if(map.get("SCORING_XIONS")!=null) iScoring_xIons = convertStringToBool(map.get("SCORING_XIONS").toString());
		if(map.get("SCORING_YIONS")!=null) iScoring_yIons = convertStringToBool(map.get("SCORING_YIONS").toString());
		if(map.get("SCORING_ZIONS")!=null) iScoring_zIons = convertStringToBool(map.get("SCORING_ZIONS").toString());
		if(map.get("SCORINGPLUGSCORING")!=null) iScoringPlugScoring = convertStringToBool(map.get("SCORINGPLUGSCORING").toString());
		if(map.get("SPECDYNRANGE")!=null) iSpectrumDynRange = Double.parseDouble(map.get("SPECDYNRANGE").toString());
		if(map.get("SPECFRAGMASSTYPE")!=null) iSpectrumFragMassType = map.get("SPECFRAGMASSTYPE").toString();;
		if(map.get("SPECMONOISOMASSERROR")!=null) iSpectrumMonoIsoMassError = Double.parseDouble(map.get("SPECMONOISOMASSERROR").toString());
		if(map.get("SPECMONOISOMASSERRORUNITS")!=null) iSpectrumMonoIsoMassErrorUnits = map.get("SPECMONOISOMASSERRORUNITS").toString();
		if(map.get("SPECFRAGMASSTYPE")!=null) iSpectrumMaxPrecursorCharge = Integer.parseInt(map.get("SPECMAXPRECURSORCHANGE").toString());
		if(map.get("SPECMINFRAGMZ")!=null) iSpectrumMinFragMz = Double.parseDouble(map.get("SPECMINFRAGMZ").toString());
		if(map.get("SPECMINPRECURSORMZ")!=null) iSpectrumMinPrecursorMz = Double.parseDouble(map.get("SPECMINPRECURSORMZ").toString());
		if(map.get("SPECMINPEAKS")!=null) iSpectrumMinPeaks = Integer.parseInt(map.get("SPECMINPEAKS").toString());
		if(map.get("SPECPARENTMASSERRORMINUS")!=null) iSpectrumParentMonoIsoMassErrorMinus = Double.parseDouble(map.get("SPECPARENTMASSERRORMINUS").toString());
		if(map.get("SPECPARENTMASSERRORPLUS")!=null) iSpectrumParentMonoIsoMassErrorPlus = Double.parseDouble(map.get("SPECPARENTMASSERRORPLUS").toString());
		if(map.get("SPECPARENTMASSERRORUNITS")!=null) iSpectrumParentMonoIsoMassErrorUnits = map.get("SPECPARENTMASSERRORUNITS").toString();
		if(map.get("SPECPARENTMASSISOERROR")!=null) iSpectrumParentMonoIsoMassIsoError = convertStringToBool(map.get("SPECPARENTMASSISOERROR").toString());
		if(map.get("SPECTRUMPATH")!=null) iSpectrumPath = map.get("SPECTRUMPATH").toString();
		if(map.get("SPECBATCHSIZE")!=null) iSpectrumSeqBatchSize = Integer.parseInt(map.get("SPECBATCHSIZE").toString());
		if(map.get("SPECTHREADS")!=null) iSpectrumThreads = Integer.parseInt(map.get("SPECTHREADS").toString());
		if(map.get("SPECTOTALPEAK")!=null) iSpectrumTotalPeakNumber = Integer.parseInt(map.get("SPECTOTALPEAK").toString());
		if(map.get("SPECUSENOISECOMP")!=null) iSpectrumUseNoiseCompression = convertStringToBool(map.get("SPECUSENOISECOMP").toString());;
	}
