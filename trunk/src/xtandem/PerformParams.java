...
public class PerformParams {
	...
	public PerformParams(HashMap map){
		...
		if(map.get("SEQSRC3") != null) iSequenceSource_3 = map.get("SEQSRC3").toString();
		if(map.get("SEQSRCDESC1") != null) iSequenceSourceDescription_1 = map.get("SEQSRCDESC1").toString();
		if(map.get("SEQSRCDESC2") != null) iSequenceSourceDescription_2 = map.get("SEQSRCDESC2").toString();
		if(map.get("SEQSRCDESC3") != null) iSequenceSourceDescription_3 = map.get("SEQSRCDESC3").toString();
		if(map.get("ESTFP") != null) iEstFalsePositives = Integer.parseInt(map.get("ESTFP").toString());
		if(map.get("NOISESUPP") != null) iSpectrumNoiseSuppressionRatio = Double.parseDouble(map.get("NOISESUPP").toString());
		if(map.get("TOTALPEPUSED") != null) iTotalPeptidesUsed = Integer.parseInt(map.get("TOTALPEPUSED").toString());
		if(map.get("TOTALPROTUSED") != null) iTotalProteinsUsed = Integer.parseInt(map.get("TOTALPROTUSED").toString());
		if(map.get("TOTALSPECASS") != null) iTotalSpectraAssigned = Integer.parseInt(map.get("TOTALSPECASS").toString());
		if(map.get("TOTALSPECUSED") != null) iTotalSpectraUsed = Integer.parseInt(map.get("TOTALSPECUSED").toString());
		if(map.get("TOTALUNIQUEASS") != null) iTotalUniqueAssigned = Integer.parseInt(map.get("TOTALUNIQUEASS").toString());
		if(map.get("PROCSTART") != null) iProcStartTime = map.get("PROCSTART").toString();
		if(map.get("PROCVER") != null) iProcVersion = map.get("PROCVER").toString();
		if(map.get("QUALVAL") != null) iQualityValues = map.get("QUALVAL").toString();
		if(map.get("INPUTMOD") != null) iInputModelNumber = Integer.parseInt(map.get("INPUTMOD").toString());
		if(map.get("INPUTSPEC") != null) iInputSpectraNumber = Integer.parseInt(map.get("INPUTSPEC").toString());
		if(map.get("PARTCLEAV") != null) iPartialCleavageNumber = Integer.parseInt(map.get("PARTCLEAV").toString());
		if(map.get("POINTMUT") != null) iPointMutationsNumber = Integer.parseInt(map.get("POINTMUT").toString());
		if(map.get("POTC_TERM") != null) iPotentialC_termNumber = Integer.parseInt(map.get("POTC_TERM").toString());
		if(map.get("POTN_TERM") != null) iPotentialN_termNumber = Integer.parseInt(map.get("POTN_TERM").toString());
		if(map.get("UNANTICLEAV") != null) iUnanticipatedCleavageNumber = Integer.parseInt(map.get("UNANTICLEAV").toString());
		if(map.get("INITMODELTOTALTIME") != null) iInitModelTotalTiming = Double.parseDouble(map.get("INITMODELTOTALTIME").toString());
		if(map.get("INITMODELSPECTIME") != null) iInitModelSpecTiming = Double.parseDouble(map.get("INITMODELSPECTIME").toString());
		if(map.get("LOADSEQMODELTIME") != null) iLoadSeqModelsTiming = Double.parseDouble(map.get("LOADSEQMODELTIME").toString());
		if(map.get("REFINETIME") != null) iRefinementTiming = Double.parseDouble(map.get("REFINETIME").toString());
	}
